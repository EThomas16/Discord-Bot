...
TODO:
    ...
    -- Fix all of the comments to be appropriate (review version on other PC)
    -- Condense play_audio method
    -- Update cat detection method call to use a more efficient method of returning the number of cats
       that have been detected
    -- General cleanup ready for next stage of implementation
"""
# necessary
import discord
import asyncio
...
class Bot():
    ...
    async def recognise_text(self, ctx, args):
        ...
        # makes sure tesseract is not passed an empty image variable
        if valid_url:
    ...
    async def scrape_image(self, url, ctx, method=""):
        """
        Uses the urllib module to retrieve the source image from a given url

        Keyword arguments:
        url -- source of image, requires an image file extension on the url
        ctx -- message pass context, contains the message and its properties
        method -- whether the scrape_image method is being called for one of two things:
                    - cat detection
                    - text recognition from an image
                    - default ""

        Returns:
        valid_url -- whether the image was retrieved from the url (Boolean)
        """
        # TODO: not used, check whether this is required
        req = Request(url, headers={'User-Agent': 'Magic Browser'})
        valid_url = False
        try:
            """ Two parameters for urlretrieve:
                    -- URL of the image to be retrieved
                    -- Destination where it will be saved (in this case the relative path)
            """
            if method == "cat":
